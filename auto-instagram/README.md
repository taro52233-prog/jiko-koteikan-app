...
flowchart TB
  ...
  D --> E["Claude API<br/>紹介文3パターン<br/>お手本の文体をfew-shotで再現"]
...
「あのアカウントみたいに書いて」を、**実物のサンプルを数本見せる（few-shot）**方式で実現している。
文体を言葉で説明するより、実投稿を見せるほうが圧倒的に効く。

`data/style-samples.json` に投稿本文を貼るだけ。URL・ハッシュタグ・メンションは自動で除去される。

```json
{
  "profile": "casual-diary",
  "samples": [
    "ここ4-7日で発送らしいので今買えばGWに使えるかなと思ってオレンジぽちった 楽天ルーム飛ぶ嫌な方はお手数ですがご自身で検索してください",
    "口つけて何度もフーフーしなくても超絶簡単に飲めるようになった"
  ]
}
```

| プロファイル | 文字数 | ハッシュタグ | 性格 |
| --- | --- | --- | --- |
| `casual-diary`（既定） | 120字 | 0個 | 独り言・買い物報告型。見出しを作らず、文の途中から始める |
| `polished` | 80字 | 5個 | 整った紹介文型。です・ます調の3拍子 |

**`casual-diary` が要求する3つのこと**（`content/style.js`）:

1. **見出しを作らない。** 「【必見】話題の〜」ではなく「ここ4-7日で発送らしいので〜ぽちった」から始める
2. **ディテールを必ず1つ入れる。** 商品ページを本当に見た人にしか書けない粒度。
   「使いやすくて便利」はこの文体では失敗扱い。「口つけて何度もフーフーしなくてもいい」が正解
3. **大げさな表現は自分の感想にだけ向ける。** 「めっちゃ助かってる」は可、「驚くほど汚れが落ちる」は不可（景表法）

サンプルからまねるのは語り口・文の始め方・ディテールの粒度・感情の出し方だけで、
**サンプル中の商品・エピソード・固有名詞の流用は禁止**するようプロンプトで明示している。

> **この文体は「本当に買っている」ことと不可分**
> 砕けた独り言が宣伝に見えないのは、書き手が実際にその商品を見て・買っているから。
> 未購入の商品に同じ文体を貼ると「ぽちった」と書けないぶん、かえって嘘くささが出る。
> そのため未所有時は自動で「ぽちるか迷ってる」「カートに入れっぱなし」の検討中レジスターに切り替わる。
> この文体を最大限に活かしたいなら、`owned-items.json` に実際に買ったものを登録していくのが本筋。
...
│   ├── content/             プロンプト・Claude呼び出し・検品・文体プロファイル(style.js)
...
├── test/                    ユニット + E2E（外部APIは全モック）54件
└── data/                    history / queue / room-log / rakuten-events / owned-items / style-samples
```
